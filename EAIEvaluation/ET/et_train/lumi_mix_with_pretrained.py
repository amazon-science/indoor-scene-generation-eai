...
{'seed': 2, 'resume': True, 'profile': False, 'batch': 8, 'epochs': 40, 
...
luminous_dataset = CustomSynthDataset("data/lmdb_new_i/", args)
...
print(embs_ann)
vocabs = [dataset.vocab_in, additional_dataset.vocab_in, additional_dataset.vocab_in]
# create the model
...
model_util.save_model(
    model, 'final_804_{}.pth'.format("save_test"), {}, optimizer=optimizer)
...
for ep in range(31):
    ...
    if valid_loss_mean < valid_best_loss:
        ...
        # save the checkpoint
        print('Saving models...')
        model_util.save_model(model, 'final_804_{}.pth'.format("best"), {}, optimizer=optimizer)
        #model_util.save_model(self, 'latest.pth', stats, symlink=True)
    ...
    if ep % 10 == 0:
        ...
        print('Saving models...')
        model_util.save_model(
            model, 'final_804_{:02d}.pth'.format(ep), {}, optimizer=optimizer)
        #model_util.save_model(self, 'latest.pth', stats, symlink=True)
