...
dataset = CustomSpeakerDataset("data/lmdb_i/", args)
...
additional_dataset = CustomSpeakerDataset("data/lmdb_synth_45k_i/", args)
additional_dataset.name = "lmdb_human"
...
valid_dataset = CustomSpeakerDataset("data/lmdb_i/", args, split="valid")
...
model_util.save_model(
    model, 'speaker_k_{:02d}.pth'.format(99), {}, optimizer=optimizer)
...
for ep in range(21):
    ...
    writer.add_scalar("train loss", np.mean(batch_mix_train_loss), ep)
    ...
    if valid_loss_mean < valid_best_loss:
        ...
        # save the checkpoint
        print('Saving models...')
        model_util.save_model(model, 'speaker_k_{}.pth'.format("best"), {}, optimizer=optimizer)
        #model_util.save_model(self, 'latest.pth', stats, symlink=True)
    ...
    if ep % 10 == 0:
        ...
        print('Saving models...')
        model_util.save_model(
            model, 'speaker_k_{:02d}.pth'.format(ep), {}, optimizer=optimizer)
        #model_util.save_model(self, 'latest.pth', stats, symlink=True)
