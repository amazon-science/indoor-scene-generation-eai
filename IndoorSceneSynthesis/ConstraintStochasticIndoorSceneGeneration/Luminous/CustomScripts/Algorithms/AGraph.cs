...
namespace Algorithm {
	...
	public class Graph {
		...
		public static Dictionary<string, Path> point2path = new Dictionary<string, Path>();
		...
		public void GetShortestPath(Node start, Node end) {
			...
			if (Graph.point2path.ContainsKey(dictPointName)) {
				m_Paths.Add(Graph.point2path[dictPointName]);
				return;
            }
			...
			Graph.point2path.Add(dictPointName, path);
			m_Paths.Add(path);
		}
