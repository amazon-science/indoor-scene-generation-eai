...
public class AlfredAgent : MonoBehaviour {
    ...
    private void ResetAgent() {
        ...
        if (!hasGraph) {
            ...
            // camera FOV
            PhysicsController.m_Camera.fieldOfView = 90f;
            ...
            for (int i = 0; i < reachablePositions.Length; ++i) {
                ...
                for (int j = 0; j < i; ++j) {
                    if (Vector3.Distance(reachablePositions[i], reachablePositions[j]) < 0.26f) {
                        graph.nodes[j].connections.Add(node);
                        node.connections.Add(graph.nodes[j]);
                    }
                }
            }
            hasGraph = true;
    ...
    public void ParseAllTaskJsonsForCurrentScene() {
        ...
        foreach (string taskJson in taskJsons) {
            ...
            if (taskJson.Contains("Shelf") || taskJson.Contains("Cabinet") || taskJson.Contains("Drawer")) {
                continue;
            }
    ...
    private void ResetObjLocations() {
        //find tool
    ...
    public void ParseTaskJson(bool deleteUnsucceeful = true) {
        ...
        string taskFolder = taskDesc.Split('/')[0];
        ...
        try {
            ...
            foreach (var s in jsonRule["script"]) {
                ...
                high_pddl.discrete_action.action = actionH;
                high_pddl.planner_action.action = actionH;
                ...
                if (actionH == "GotoLocation") {
                    ...
                    GoToLocation(targetLoc, low_idx, rawImageFolder);
    ...
    public void Debugg() {
        ...
        ParseTaskJson(deleteUnsucceeful: false);
    }
    ...
    public Vector3 ModifyLastLocation(Vector3 targetLocation) {
        Vector3 endLoaction = graph.GetNearestNode(targetLocation).node_position;
    ...
    //high-level action
    public void GoToLocation(Vector3 targetLocation, int frameStart = 0, string saveImageFolder = "Assets/Custom/Screenshots/") {
        //actionList.Clear();
        ...
        Node startNode = graph.GetNearestNode(currentLocation);
        Node endNode = graph.GetNearestNode(targetLocation);

        graph.GetShortestPath(startNode, endNode);
        Algorithm.Path path = graph.m_Paths[0];
    ...
    private void ToggleObjectOn(string objectId) {
        ...
        PhysicsController.ToggleObjectOn(objectId: objectId, forceAction: true);
    }

    private void ToggleObjectOff(string objectId) {
        PhysicsController.agentState = AgentState.Processing;
        PhysicsController.ToggleObjectOff(objectId: objectId, forceAction: true);
    }

    private void PickUpObject(string objectId) {
        PhysicsController.agentState = AgentState.Processing;
        PhysicsController.PickupObject(objectId: objectId, forceAction: true);
    ...
    private void PutObject(string objectId) {
        PhysicsController.agentState = AgentState.Processing;
        PhysicsController.PutObject(objectId: objectId, forceAction: true);
